...
const setPackageVersions = (
	...
	versionedPackages.forEach((pkgName) => {
...
const updatePkgDeps = (
	...
	if (pkgFile) {
		...
		if (currVer) {
			...
			if (currVer !== newVer) {
				console.log(
					"Found linked dependency to update:",
					packageName,
					currVer,
					newVer,
				);
...
const updateRenativeDeps = (
	...
	const doUpdate = (obj: any, key: string) => {
		...
		if (templateVer) {
			if (templateVer !== newVer) {
				console.log(
					"Found linked plugin dependency to update:",
					packageName,
					templateVer,
					newVer,
				);
	...
	if (rnvTempFile) {
		doUpdate(
			rnvTempFile?.templateConfig?.package_json?.devDependencies,
			packageName,
		);
		if (hasRnvChanges) {
...
export const prePublish = async (c: RnvContext) => {
	...
	_updateJson(
		path.join(pkgDirPath, "config-templates/renative.templates.json"),
		{
			engineTemplates: {
				"@rnv/engine-rn": v,
			},
	...
	copyFileSync(
		...
		path.join(pkgDirPath, "renative/README.md"),
	);
	copyFileSync(
		path.join(c.paths.project.dir, "README.md"),
		path.join(pkgDirPath, "renative/README.md"),
	...
	copyFileSync(
		path.join(c.paths.project.dir, "README.md"),
		path.join(pkgDirPath, "rnv/README.md"),
	);
};
